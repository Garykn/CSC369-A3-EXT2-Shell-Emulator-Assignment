...
struct ext2_inode *pn;  
	...
	*/
	void loadImage(char * diskimage){
		...
		// set global pointer disk to point to the beginning of the disk image in memory
	...
	*/
	struct comp_inode_type * getObjectInode(char * tokens[], struct comp_inode_type *comp_i_t){
		...
		if(tokens[0]==NULL){//quick check to see if root's inode was requested
	...
	INPUT: 	sub_component - name of child to search
	...
	*/
	void getChildInode(char * sub_component, int parent_inode, int dirCheck, struct comp_inode_type *comp_i_t){
		...
		for (i=0; i< numOfBlocks; i++){
			...
			while(offset < EXT2_BLOCK_SIZE){
				if(strncmp(dblock_p->name,sub_component, dblock_p->name_len) == 0){//match found
	...
	/*
	Description: Insert a record into a parent directory. 
	INPUT: 	parent_inode - parent inode - inode of parent directory
	...
	*/
	...
	/*
	Description: delete the object with inode comp_inode
	...
	void delObject(int parent_inode, int comp_inode){
		...
		for (i=0; i< numOfBlocks; i++){
			...
			while(offset < EXT2_BLOCK_SIZE){
				...
				if(dblock_p->inode == comp_inode){// if child record found
					...
					if(prev_entry == NULL){
						...
						/*Parent directory has previous entries in the data block, 
						delete by making this directory entry a padding for 
					...
					//deallocate all data blocks of the deleted file in  block bitmap
