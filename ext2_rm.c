...
int main(int argc, char **argv) {
	...
	//get path of parent directory and name of component to remove
    ...
    if(check_ibitmap(comp_it_p->inode) == 0){// if doesn't exist
		fprintf( stderr, "component doesn't exist");
		printf("inode of component is: %d", comp_it_p->inode);
	...
	if(comp_it_p->inode == 0) {// component not found!
		fprintf( stderr, "component not found");
		printf("inode of component is: %d", comp_it_p->inode);
		fprintf( stderr, strerror(ENOENT));
		fprintf( stderr, "\n");
		exit(1);
	...
	//component must not be directory - we're not allowed to delete directories
